...
def disparity_to_depth(disparity):
    ...
    x = 0
    y = 0
    depth = np.zeros((370,1226))

    for x in range(0, 1225):
        for y in range(0, 369):
            #print(depth[y,x])
            disparity_point = disparity[y,x]
            #print(disparity_point)
            if disparity_point > min_disparity and disparity_point > 13000:
                try:
#                   depth = baseline * f_x / disparity_point
                    depth[y,x] = 0.54 * 721 * 1000 / disparity_point
                except ValueError:
                    depth[y,x] = 0
            else:
                depth[y,x] = 0
